...
export function coletoGround(world) {
  ...
  return {
    ...
    // Each corner gun faces out from its corner; the white flag at the front corner (`phase.flags`).
    'corner-fl-out': at(C, 0.6, 0.6), 'corner-fr-out': at(C, 0.6, -0.6), 'corner-bl-out': at(C, -0.6, 0.6), 'corner-br-out': at(C, -0.6, -0.6),
    flag: at(C, 0.085, 0.13),
...
/**
 * Urrea's men in an assault (sim/battle-stage.mjs `phase.groups`): the Jiménez battalion in ranks in front is the side's own
 * body, firing by volleys; the riflemen on the left, the grenadiers and San Luis on the right and the cavalry at the rear are
 * drawn apart from it, each on its keyframes out from its far point to its near one and back.
 */
const assault = ({ left, right, front, rear }) => ({
  mexican: { style: 'ranks', action: 'advance', fire: 'volley', count: 280, drawn: 18, keys: front.map(([m, p]) => [m, `front-${p}`]) },
  groups: [
    { id: 'left', side: MEX, name: 'Riflemen', style: 'loose', fire: 'scattered', drawn: 14, spread: { width: 0.26, depth: 0.08 }, keys: left.map(([m, p]) => [m, `left-${p}`]), face: 'square' },
    { id: 'right', side: MEX, name: 'Grenadiers', style: 'column', fire: 'scattered', drawn: 14, keys: right.map(([m, p]) => [m, `right-${p}`]), face: 'square' },
    { id: 'rear', side: MEX, name: 'Cavalry', style: 'mounted', fire: 'picket', mounted: true, drawn: 11, keys: rear.map(([m, p]) => [m, `rear-${p}`]), face: 'square' },
  ],
});
/**
...
/** Four of the nine brass guns, at the square's corners, each facing out from its corner. */
const CORNERS = ['corner-fl', 'corner-fr', 'corner-bl', 'corner-br'];
/** The guns standing in a phase (sim/battle-stage.mjs `phase.guns`): each with the minutes it fires at. */
const guns = (shots = {}, { battery = false } = {}) => Object.fromEntries([...CORNERS, ...(battery ? ['battery-1', 'battery-2'] : [])].map(id => [id, shots[id] || []]));
...
export const COLETO = Object.freeze({
  ...
  sides: {
    ...
    mexican: { name: 'Urrea’s division', count: 280, drawn: 18, claimId: 'HIST-TEX-515' },
  },
  // Four of the nine brass guns, at the square's corners (the rest stood in its faces and are not drawn apart; ceiling: four for
  // nine), and the Mexican battery that came up in the night, in front by the timber (sim/battle-stage.mjs `def.guns`).
  guns: [
    ...CORNERS.map(at => ({ id: at, side: TEX, at, face: `${at}-out`, metal: 'bronze', crew: 3, claimId: 'HIST-TEX-515' })),
    { id: 'battery-1', side: MEX, at: 'battery-1', face: 'square', metal: 'iron', crew: 3, claimId: 'HIST-TEX-515' },
    { id: 'battery-2', side: MEX, at: 'battery-2', face: 'square', metal: 'iron', crew: 3, claimId: 'HIST-TEX-515' },
  ],
  flag: null,
  ...
  phases: [
    ...
    },
    {
      ...
      mexican: { style: 'column', at: 'urrea', action: 'gone', fire: 'none' },
      groups: [{ id: 'horton', side: TEX, name: 'Horton’s horsemen', style: 'loose', drawn: 5, keys: [[0, 'out-ahead'], [90, 'halt-ahead'], [150, 'halt-ahead'], [180, 'timber']], spread: { width: 0.08, depth: 0.05 }, face: 'square', action: 'advance' }],
      lines: [
    ...
    },
    {
      ...
      groups: [
        { id: 'horton', side: TEX, name: 'Horton’s horsemen', style: 'loose', drawn: 5, from: 'timber', to: 'horton-gone', spread: { width: 0.08, depth: 0.05 }, face: 'horton-gone', action: 'withdraw' },
        { id: 'left', side: MEX, style: 'mounted', mounted: true, drawn: 10, keys: [[0, 'rear-road'], [10, 'left-far']], action: 'advance', face: 'square' },
      ],
      lines: [
    ...
    },
    {
      ...
      guns: guns({ 'corner-fl': [17, 31], 'corner-fr': [19], 'corner-bl': [24], 'corner-br': [26, 36] }),
      falls: [
        ...
        { side: MEX, count: 2, at: 21, claimId: 'HIST-TEX-515' },
        { side: MEX, unit: 'rear', count: 1, at: 26, claimId: 'HIST-TEX-515', wounded: true, carried: true },
        { side: MEX, unit: 'right', count: 1, at: 29, claimId: 'HIST-TEX-515', wounded: true, carried: true },
        { side: MEX, unit: 'left', count: 1, at: 33, claimId: 'HIST-TEX-515' },
      ],
      lines: [
        say('k-here', 8, TEX, 'volunteer', 'reconstructed', 'Here they come, all round!'),
        say('k-carga', 22, MEX, 'officer', 'reconstructed', '¡A la carga!', { gloss: 'Charge!', unit: 'rear' }),
    ...
    },
    {
      ...
      guns: guns({ 'corner-bl': [17], 'corner-br': [21], 'corner-fl': [25], 'corner-fr': [32] }),
      falls: [
        { side: TEX, count: 1, at: 20, claimId: 'HIST-TEX-063' },
        { side: TEX, count: 2, at: 24, claimId: 'HIST-TEX-063', wounded: true },
        { side: MEX, unit: 'left', count: 1, at: 22, claimId: 'HIST-TEX-515' },
        { side: MEX, unit: 'rear', count: 1, at: 24, claimId: 'HIST-TEX-515', wounded: true, carried: true },
        { side: MEX, unit: 'right', count: 1, at: 30, claimId: 'HIST-TEX-515', wounded: true, carried: true },
      ],
      lines: [
        say('k-again', 6, TEX, 'volunteer', 'reconstructed', 'They’re coming again!'),
        say('k-cav', 20, MEX, 'officer', 'reconstructed', '¡Caballería, a la carga!', { gloss: 'Cavalry, charge!', unit: 'rear' }),
    ...
    },
    {
      ...
      guns: guns({ 'corner-fr': [19], 'corner-fl': [22], 'corner-br': [30] }),
      falls: [
        { side: TEX, count: 1, at: 26, claimId: 'HIST-TEX-063', wounded: true },
        { side: MEX, count: 1, at: 24, claimId: 'HIST-TEX-515' },
        { side: MEX, unit: 'left', count: 1, at: 28, claimId: 'HIST-TEX-515', wounded: true, carried: true },
      ],
      lines: [
    ...
    },
    {
      ...
      guns: guns({ 'battery-1': [0, 11, 24], 'battery-2': [4, 16, 31], 'corner-fl': [8], 'corner-fr': [20] }, { battery: true }),
    ...
    },
    {
      ...
      groups: [{ id: 'riders', side: MEX, style: 'mounted', mounted: true, drawn: 12, from: 'back-right', to: 'gate-right', face: 'goliad', action: 'follow' }],
...
/**
 * Where a family's man with Fannin stands in each phase: which body of the force (the side's own, `texian`, or Horton's
 * horsemen drawn apart from it), and his place in it. Kept here beside the data so the director and the renderer agree.
 * Returns `{ unit, along, across }` in miles from that body's middle, in the frame whose `along` runs the way it faces; in the
 * square, `face` is the side of it he stands in, in its front rank (public/battle-view.js `SQUARE.outer`).
 */
export function coletoSlot(personId, index, { horton, phase }) {
  ...
  if (['march-out', 'road', 'caught'].includes(phase)) {
    if (horton) return { unit: 'horton', along: -0.02 * a, across: (b - 0.5) * 0.06 };
    // In the column's files, somewhere along it.
    return { unit: 'texian', along: -0.04 - 0.3 * a, across: ((index % 4) - 1.5) * 0.022 };
  }
  if (phase === 'march-back') return { unit: 'texian', column: true, along: -0.04 - 0.3 * a, across: ((index % 4) - 1.5) * 0.022 };
  const face = ['front', 'left', 'right', 'rear'][Math.floor(a * 4) % 4];
  const t = (b - 0.5) * 0.08, out = 0.09;
  return { unit: 'texian', face, ...(face === 'front' ? { along: out, across: t } : face === 'rear' ? { along: -out, across: t } : face === 'left' ? { along: t, across: out } : { along: t, across: -out }) };
}
