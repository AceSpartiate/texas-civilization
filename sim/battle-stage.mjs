...
export const ENGAGEMENTS = Object.freeze({ [GONZALES.id]: GONZALES, [BEXAR_STORMING.id]: BEXAR_STORMING, [CONCEPCION.id]: CONCEPCION, [GRASS_FIGHT.id]: GRASS_FIGHT });
  ...
  if (phase.groups?.length) {
    view.groups = phase.groups.map(group => {
      ...
      return {
        ...
        ...(group.figure && { figure: group.figure }),
      };
    });
  }
