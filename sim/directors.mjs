...
export function directorProjection(world, householdId, role, { seen = [] } = {}) {
  ...
  // Coleto and Palm Sunday (sim/fannin.mjs), by the same rules; none of these fights is ever fought at the same time as another.
  ...
  const shownAlert = bexar?.battleAlert || fannin?.battleAlert || battleAlert || south?.alert || null, shownAccount = bexar?.battleAccount || fannin?.battleAccount || battleAccount || south?.account || null;
