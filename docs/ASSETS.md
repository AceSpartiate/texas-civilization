...
Rust is reserved for the local principal; the other palettes are stable per-ID choices. An observed household's principal never borrows the local principal's mark. Clip selection for an observed person cannot infer their private chore or cargo. Injured/wounded hold injury art. Capture and death retain the current non-graphic still fallback and separate textual state; a sprite must not decide those outcomes.
