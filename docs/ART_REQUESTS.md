...
Still requested for the named-person roster: mounted Houston and Santa Anna; Crockett's capture/surrender and the disputed, non-graphic fate presentation chosen in `docs/BATTLES.md` §2c; Travis's fall and Joe's subsequent movement; full mounted movement for Seguín; the remaining Tejano/Tejana cast; Austin, Burleson, Cos, Moore, Castañeda, Urrea, Castrillón, Almonte, and the other named people in `docs/battle-research/famous-people.md`. These need the same identity-preserving directional walks and the specific authored actions of their itinerary, with no generic figure silently presented as their portrait. The remaining gameplay work, including who is visible and what is known at a given date, is separate from art production.
...
| A family's appearance is shown through the painted cast atlases with a cached palette pass; some head styles still use the nearest cast silhouette | `public/avatar-art.js`, `public/person-palette.js`, `public/appearance.js` | Request below — aligned painted people layers | Exact headwear and hair variants in every animation, plus semantic masks for reliable palette changes |
...
| **Fog** at Concepción is a pale radial veil drawn over the field at the phase's density | `drawFog` in `public/battle-view.js` (`stand-in:`) | Request 2026-09-25 — Concepción and the Grass Fight, item 3 | `fog-bank` sprites, dense and thinning, that lie on the ground and drift |
...
| **A Mexican bugler** sounding the charge and the retreat is the officer's bubble with the call in brackets (`[The bugle sounds the charge]`) over the infantry | the `bugler` lines of `sim/battles/concepcion.mjs` drawn by `drawLines` (`stand-in:`) | Request 2026-09-25 — Concepción and the Grass Fight, item 5 | `regular-bugler` (idle and sounding), `regular-drummer` |
| **The pack train** at the Grass Fight is `horse-walk`/`horse-graze` with `packed-belongings` drawn on its back | the `packhorse` figure in `draw`, `public/battle-view.js` (`stand-in:`) | Request 2026-09-25 — Concepción and the Grass Fight, item 6 | `mule-packed-grass-walk` (east, north, south) and `grass-bundle-cut` |
...
- **Layered people.** Students now choose what parents look like (skin tone, hair colour, clothing colour,
  ...
  aligned layers for the missing precise head styles and more reliable colour regions across
  ...
  **Updated 2026-09-26 for the atlas-based appearance renderer** (`sim/look-vocabulary.mjs`,
  ...
  - **What Claude wires on delivery:** replace the current pixel classifier in `public/person-palette.js` with
...
5. **A bugler and a drummer** `regular-bugler` (idle, sounding) and `regular-drummer`, so the charge and the retreat are sounded by somebody (`HIST-TEX-480`); the call stays captioned. No Texian drum or flag (Creed Taylor).
