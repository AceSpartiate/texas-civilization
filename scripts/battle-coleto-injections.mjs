...
const UNIT = [
  { name: 'the column never leaves Goliad: the men wait at the presidio and cannot reach the square in time', file: 'sim/fannin.mjs',
    from: '    const centre = unitPlace(ground, phase, slot.unit, into);', to: '    const centre = ground.goliad;',
  ...
  { name: 'the dead man is marched back to Goliad with the prisoners', file: 'sim/fannin.mjs',
    from: "    if (fellAt(battle, person.id, 'killed', world.minute)) continue;", to: '',
    test: COLETO, expect: 'each man\'s fate falls at its moment inside the fighting, from the roll the game always made; a wound shows at once, a death waits for the word, and the dead man lies where he fell' },
  ...
  { name: 'a fallen man moves with his company', file: 'public/battle-view.js',
    from: '        const ground = down ? view.fallenSpots.get(seedKey) : at;', to: '        const ground = at;',
  ...
  { name: 'dusk drawn as day', file: 'public/battle-view.js',
    from: '    if (DIM[battle.light] && bounds) {', to: '    if (false) {',
