...
try{const page=await browser.newPage({viewport:{width:1440,height:1000}}),errors=[];page.on('pageerror',e=>errors.push(e.message));
  ...
  // The family is made before the world is drawn (public/creation.js, owner 2026-09-17): title, die, names, looks. The old
  // die-then-journal steps this proof pressed were replaced by that walk; scripts/support/meet-family.mjs walks it.
  assert.equal(await meetFamily(page),'Proofwright','the family-making walk was not shown');
