...
export const measureStep = (page, panelSelector, endSelector) => page.evaluate(({ selector, finger, endOf }) => {
  ...
  //
  // **Trap (c), found when the two wizard passes of 2026-09-21 were merged.** A control clipped at the fold of a list
  // that scrolls keeps its whole layout box - `inBox` is an intersection, so the control is rightly still counted as
  // present - and measuring a gap from the part of it that is under the fold measures a gap to something nobody can
  // see or press. The last row of the looks card read as 2.7px from Done that way, and no amount of margin moved the
  // number, because the margin was never what the number was about. Gaps are measured from what is drawn: each box is
  // cut to its clipping ancestor first.
  const shown = control => {
  ...
  for (let i = 0; i < targets.length; i++) {
    for (let j = i + 1; j < targets.length; j++) {
      const a = shown(targets[i]), b = shown(targets[j]);
      ...
      if (gap < 0) continue;
      if (!tightest || gap < tightest.gap) tightest = { gap: Math.round(gap * 10) / 10, between: [name(targets[i]), name(targets[j])], labels: [(targets[i].textContent || '').trim().slice(0, 20), (targets[j].textContent || '').trim().slice(0, 20)], boxes: [rect(a), rect(b)], tags: [targets[i].tagName.toLowerCase(), targets[j].tagName.toLowerCase()] };
    }
  }
