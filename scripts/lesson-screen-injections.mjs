...
//
// Run: node scripts/lesson-screen-injections.mjs  → writes docs/evidence/lesson-screen-injections.json
...
writeFileSync('docs/evidence/lesson-screen-injections.json', `${JSON.stringify({ record: 'lesson-screen-injections', date: new Date().toISOString().slice(0, 10), files: FILES, injections: record }, null, 2)}\n`);
console.log(`\n${record.filter(r => r.failed.length).length} of ${record.length} caught; wrote docs/evidence/lesson-screen-injections.json`);
