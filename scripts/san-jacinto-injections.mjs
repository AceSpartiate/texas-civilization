...
const TESTS = {
  ...
  camp: 'a camp at rest is scattered and unformed, some standing and some sitting, and fires nothing; the formed line against it stands in even ranks',
  ...
  guns: 'each gun fires on its own dated shots, stays at its station once pinned, and its crew is its own side\'s',
  party: 'a party of horse is drawn apart from its side, as riders, with its own fall; the breastwork, the fires and the marsh stand on the ground',
  down: 'a family\'s own man down is drawn so from the minute the server says: lying still if killed, sitting hurt if wounded, and firing no more',
};
const UNIT = [
  ...
  { name: 'a joiner is given the camp he never reached, and stands at the old one', file: 'sim/winter.mjs',
    from: "  const siteId = kind === 'houston' ? (entity.location?.siteId || houstonCamp(world)) : terms.siteId;", to: "  const siteId = kind === 'houston' ? houstonCamp(world) : terms.siteId;", test: T, expect: TESTS.joiner },
  ...
  { name: 'a man\'s fall is sent to every family', file: 'sim/san-jacinto.mjs',
    from: '    out.battle = { ...projectBattle(world, ID, { members, memberStates: memberStates(world, householdId) }), reconstruction: false };',
    to: '    out.battle = { ...projectBattle(world, ID, { members, memberStates: memberStates(world, null) }), reconstruction: false };', test: T, expect: TESTS.fate },
  ...
  { name: 'a later fall lands on a man already down', file: 'public/battle-view.js',
    from: '      const order = [...slots].filter(slot => !map.has(slot.index)).sort(', to: '      const order = [...slots].sort(', test: V, expect: TESTS.rout },
  { name: 'only the first gun is drawn', file: 'public/battle-view.js',
    from: '    for (const gun of gunsIn(battle)) { const shown = drawCannon(', to: '    for (const gun of gunsIn(battle).slice(0, 1)) { const shown = drawCannon(', test: V, expect: TESTS.guns },
  { name: 'a party is counted into its side', file: 'public/battle-view.js',
    from: '        (party ? speakers : drawn)[side.side].push(point);', to: '        drawn[side.side].push(point);', test: V, expect: TESTS.party },
  ...
  { name: 'a family\'s man killed goes on firing', file: 'public/battle-view.js',
    from: "    if (member.down === 'killed') return { sprite: 'volunteer-reclining', flip: !right, still: true };\n", to: '', test: V, expect: TESTS.down },
];
