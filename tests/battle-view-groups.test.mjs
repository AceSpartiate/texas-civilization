...
// The merge of San Jacinto and Concepción (2026-09-26): each fall takes men still up (San Jacinto's many falls add up) and the
// men it took are kept, so they still lie there once their body has left the field (Concepción's Coleman's men).
test('falls in one body add up, never landing on a man already down, and all of them still lie there after it has left the field', () => {
  const falls = [1, 2, 3, 4].map(minute => ({ side: 'texian', unit: 'coleman', count: 4, minute, carried: false }));
  const make = minute => battle(minute, {
    fallen: falls.filter(fall => fall.minute <= minute),
    groups: minute < 7 ? [group('coleman', 'texian', 'loose', 'scattered', -0.1, { drawn: 20, y: -0.12, spread: { width: 0.2, depth: 0.08 } })] : [],
  });
  const art = fakeArt(), view = createBattleView(art);
  const before = run(view, make, 5.5);
  assert.equal(before.fallenBy['g:coleman'], 16, `a later fall landed on a man already down: ${before.fallenBy['g:coleman']} of 16`);
  art.drawn.length = 0;
  const after = run(view, make, 3, 7500);
  assert.equal(after.fallenBy['g:coleman'], 16, `the fallen of a body that left the field are not all kept: ${after.fallenBy['g:coleman']}`);
  const lying = new Set(art.drawn.filter(one => one.sprite === 'volunteer-reclining').map(one => `${Math.round(one.x)}:${Math.round(one.y)}`));
  assert.equal(lying.size, 16, `${lying.size} of the 16 are drawn lying where they fell once their body has gone`);
});
