...
test('what a person may be asked to do is decided on the server, with a reason', () => {
  ...
  // And the stock's three (sim/stock.mjs), which a family with no herd is not shown at all: this family drove none in.
  // And the children's six (sim/children.mjs, 2026-09-21), which are nobody's but a person under ten's: Thomas is a man.
  // And the old walk to the shops, kept only for a class saved in the middle of it and offered to nobody (2026-09-24).
  assert.equal(offered.length, Object.keys(CHORES).filter(id => !CHORES[id].retired && !CHORES[id].house && !CHORES[id].helps && !CHORES[id].well && !CHORES[id].lane && !CHORES[id].fells && !CHORES[id].fetchesLogs && !CHORES[id].hauling && !CHORES[id].winter && !CHORES[id].road && !CHORES[id].camp && !CHORES[id].directorOnly && !CHORES[id].forage && !CHORES[id].stock && !CHORES[id].child && id !== 'clear-plot').length + offered.filter(entry => CHORES[entry.id].forage || CHORES[entry.id].stock).length, 'every chore is accounted for, refused or not');
  ...
  // A family that brought no stock is offered nothing about stock: not a refusal, an absence.
  assert.equal(offered.filter(entry => CHORES[entry.id].stock).length, 0, 'a family with no herd was offered a beef to kill');
