...
test('what a person may be asked to do is decided on the server, with a reason', () => {
  ...
  assert.equal(offered.length, Object.keys(CHORES).filter(id => !CHORES[id].house && !CHORES[id].helps && !CHORES[id].well && !CHORES[id].lane && !CHORES[id].fells && !CHORES[id].hauling && !CHORES[id].winter && !CHORES[id].camp && id !== 'clear-plot').length, 'every chore is accounted for, refused or not');
