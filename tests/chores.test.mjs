...
test('what a person may be asked to do is decided on the server, with a reason', () => {
  ...
  // nobody needs either.
  assert.equal(offered.length, Object.keys(CHORES).filter(id => !CHORES[id].house && !CHORES[id].helps && !CHORES[id].well && !CHORES[id].lane).length, 'every chore is accounted for, refused or not');
