...
test('what a person may be asked to do is decided on the server, with a reason', () => {
  ...
  // nobody needs either; and clearing, for a family with nothing staked to clear (sim/fields.mjs).
  assert.equal(offered.length, Object.keys(CHORES).filter(id => !CHORES[id].house && !CHORES[id].helps && !CHORES[id].well && !CHORES[id].lane && id !== 'clear-plot').length, 'every chore is accounted for, refused or not');
