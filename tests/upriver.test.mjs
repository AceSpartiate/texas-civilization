  ...
  for (let tick = 0; tick < 400 && !world.director.complete; tick++) {
    ...
    for (const id of ids) {
      ...
      if (request?.status === 'open' && request.kind !== 'march' && !helped.has(id)) {
        ...
        try { applyAction(world, id, { action: request.kind === 'rumor' ? 'go-see' : 'help', entityId: world.households[id].principalId, mode: 'foot' }); if (request.kind !== 'rumor') helped.add(id); } catch { /* not yet known */ }
      }
      if (request?.kind === 'march' && request.status === 'open' && !answered.has(id) && march) {
        ...
        applyAction(world, id, { action: march, entityId: request.actorId, mode: 'foot' });
      }
    }
    onTick?.(world, tick);
  }
...
test('the call to go upriver is only put to a family whose person is actually standing in Gonzales', () => {
  ...
  for (let tick = 0; tick < 400 && !world.director.complete; tick++) {
    ...
    if (offered?.status === 'open' && offered.kind !== 'march' && !sent) {
      try { applyAction(world, 'hh-2', { action: offered.kind === 'rumor' ? 'go-see' : 'help', entityId: world.households['hh-2'].principalId, mode: 'foot' }); if (offered.kind !== 'rumor') sent = true; } catch { /* not yet known */ }
    }
