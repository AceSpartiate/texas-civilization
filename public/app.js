...
// (`action: 'gone'`: the Mexicans gone toward Béxar at Concepción).
const battlePoints = world => (world.battle?.frame?.length ? world.battle.frame : [...[...(world.battle?.sides || world.battle?.formations || []), ...(world.battle?.groups || [])].filter(body => body.action !== 'gone'), ...(world.battle?.cannon ? [world.battle.cannon] : [])]).map(point => ({ x: point.x, y: point.y }));
