...
export function createBattleView(art) {
  ...
  function accept(battle, now, tickMs) {
    ...
    if (view.key !== key) {
      ...
      view.gunFiredAt.clear(); view.breachAt.clear(); view.memberFallAt.clear(); view.fallenSpots.clear(); view.fallenSide.clear(); view.herd = null;
      view.minute = null;
    }
  ...
  function draw(ctx, battle, { camera, time, now, tickMs, wind = null, reducedMotion = false, paused = false, bounds = null, named = false }) {
    ...
    for (const side of bodies) {
      ...
      for (const slot of slots) {
        ...
        // A man who fell lies where he fell, whatever his side or part does after, and after it has left the field (§6.13, §8).
  ...
  function fallenBySide(battle, now) {
    ...
    for (const fall of view.fallenAt.values()) {
      ...
      if (!fall.slots) {
        ...
        fall.slots = up.slice(0, fall.count).map(slot => slot.index); fall.part = Boolean(side.part);
      }
