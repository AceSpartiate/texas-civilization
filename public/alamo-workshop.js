...
let damage=createDamageState(),mode='cutaway',playing=true,time=0,last=0,paint=0,selected='scale-person',study=false;
...
function drawWall(w){
  ...
  for(const segment of wallSections(w)){
    ...
    const face=[a,b,{x:b.x,y:b.y-rise},{x:a.x,y:a.y-rise}];polygon(face,w.material==='timber'?'#927449':'#c9b895','#76664b');
    const sprite=w.material==='timber'?'alamo-palisade':health<100?'alamo-wall-cracked':'alamo-wall-intact';
    ...
    if(frame&&camera.scale>.75){for(let i=0;i<count;i++){
      ...
      ctx.save();polygon([start,end,{x:end.x,y:end.y-rise},{x:start.x,y:start.y-rise}],'#c8b794');ctx.clip();
    ...
    polygon([{x:a.x,y:a.y-rise},{x:b.x,y:b.y-rise},{x:b.x+thick,y:b.y-rise-thick*.45},{x:a.x+thick,y:a.y-rise-thick*.45}],w.material==='timber'?'#b39a69':'#e1d3ad','#8b7b5c');
  }
}
function draw(){
  ...
  if($('#show-labels').checked){for(const note of layout.annotations){const p=project(note);label(note.label,p.x,p.y-12,camera.scale>2?13:10);}if(camera.scale>3)for(const r of layout.rooms){const p=project(roomCenter(r));label(r.label,p.x,p.y,11);}}
  ...
  window.__alamo={ready:true,units:layout.units,rooms:layout.rooms.length,wallStates:{...damage.walls},actors:structuredClone(actors),camera:{...camera},roofMode:mode,timeMs:time,scaleHeightFeet:layout.referenceHeightFeet};
}
...
$('#fit-compound').onclick=()=>{focusRoom('');draw();};$('#room-focus').onchange=e=>{focusRoom(e.target.value);draw();};
