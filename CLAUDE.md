...
16. `docs/TOWNS.md` §4b before changing the errand to town, what an order that needs the wagon, the horse, the ox or the rifle does, or `sim/keeping.mjs` — the errand chosen in a popup before anybody leaves, the server choosing the way of going from the load, and **exclusive use of family property** (one person at a time, released when the work or journey ends) are owner-decided (2026-09-24).
